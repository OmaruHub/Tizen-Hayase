...
export default new class Attachments {
  ...
  _metadata (hash: string, id: number) {
    const file = this.filemap.get(hash + id) || this.filemap.get(hash.toLowerCase() + id) || this.filemap.get(hash.toUpperCase() + id)
    if (!file) return
  ...
  subtitle (hash: string, id: number, cb: (subtitle: { text: string, time: number, duration: number }, trackNumber: number) => void) {
    ...
    // Replay any already-parsed subtitles so initial dialogue is never missed
    if (metadata.parsedSubtitles && metadata.parsedSubtitles.length > 0) {
      for (const { subtitle, track } of metadata.parsedSubtitles) {
        try {
          cb(subtitle, track)
        } catch {}
      }
  ...
  register (torrent: Torrent) {
    ...
    torrent.files.forEach((file, id) => {
      ...
      if (lname.endsWith('.mkv') || lname.endsWith('.webm')) {
        this.filemap.set(hash + id, file)
        this.filemap.set(torrent.infoHash + id, file)
