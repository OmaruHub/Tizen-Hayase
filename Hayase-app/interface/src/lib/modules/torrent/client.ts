...
export const server = new class ServerClient {
  ...
  constructor () {
    ...
    if (last) {
      this.playHash(last.id, last.media, last.episode, last.id, false)
    ...
    if (typeof window !== 'undefined') {
      ;(window as any).__hayaseTorrentServer = this
    }
  }
  ...
  playHash (infoHash: string, media: Media, episode: number, torrent: string | ArrayBufferView = infoHash, autoNavigate = true) {
    ...
    if (autoNavigate && (SUPPORTS.isTV || SUPPORTS.isTizen || SUPPORTS.isTizenTV)) {
      goto('/#/app/player')
    }
