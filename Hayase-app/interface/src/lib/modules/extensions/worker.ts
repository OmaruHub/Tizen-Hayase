import { finalizer, releaseProxy } from 'abslink'
...
export class ExtensionWorker {
  ...
  constructor (public name?: string) {
    ...
    this[finalizer] = this[finalizer].bind(this)
  ...
  async load (code: string, type: 'torrent' | 'nzb' | 'subtitle' | 'http'): Promise<TSource> {
    // Sanitize modern optional chaining in extension bundles for Chrome 76 compatibility
  ...
  async url () {
    if (!this._state) await this.loaded()
