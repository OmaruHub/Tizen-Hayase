...
import SUPPORTS from '../settings/supports.ts'

import Worker from './worker?worker'
import { ExtensionWorker } from './worker.ts'
...
export const safejs = async (url: string): Promise<string | null> => {
  try {
    ...
    text = text.replace(/r\.date\?\.getTime\?\.\(\)/g, '(r.date && r.date.getTime ? r.date.getTime() : undefined)')
    return text
  } catch (e) {
    return null
  }
...
class CodeManager {
  ...
  async _loadWorker (code: string, id: string, type: 'torrent' | 'nzb' | 'subtitle' | 'http', showErrors = true) {
    ...
    code = code.replace(/r\.date\?\.getTime\?\.\(\)/g, '(r.date && r.date.getTime ? r.date.getTime() : undefined)')

    let Loader: Remote<typeof extensionLoader>
    if (SUPPORTS.isTV || SUPPORTS.isTizenTV || SUPPORTS.isTizen) {
      debug('TV platform detected, using in-thread ExtensionWorker for', id)
      Loader = new ExtensionWorker(id) as unknown as Remote<typeof extensionLoader>
    } else {
      try {
        Loader = wrap<typeof extensionLoader>(new Worker({ name: id })) as unknown as Remote<typeof extensionLoader>
      } catch (_e) {
        debug('Worker constructor threw, falling back to in-thread ExtensionWorker for', id)
        Loader = new ExtensionWorker(id) as unknown as Remote<typeof extensionLoader>
      }
    }
...
export const storage = new class ConfigManager {
  ...
  constructor () {
    ...
    if (typeof window !== 'undefined') {
      window.addEventListener('hayase-install-extension', async (e: any) => {
        const url = e.detail?.url || e.detail
        if (url && typeof url === 'string') {
          try {
            toast.info('Importing extension from companion...', { description: url })
            await this.import(url)
            toast.success('Extension imported successfully!')
          } catch (err: any) {
            toast.error((err && (err.cause || err.message)) || 'Extension Import Failed', { description: err?.message, duration: 15_000 })
          }
        }
      })
    }
  ...
  async autoSeedDefaultRepo () {
    const currentConfigs = this.configs()
    if (Object.keys(currentConfigs).length === 0) {
      try {
        debug('Auto-seeding default extensions repository...')
        await this.import('https://raw.githubusercontent.com/anh9000/anitorrent/main/hayase/index.json')
        debug('Default extensions successfully auto-seeded!')
      } catch (err) {
        debug('Auto-seeding skipped or notice:', err)
      }
    }
  }
  ...
  async update () {
    await this.ready
    await this.autoSeedDefaultRepo()
