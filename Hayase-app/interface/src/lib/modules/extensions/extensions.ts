...
export const extensions = new class Extensions {
    ...
    for (const [id, worker] of extensions.entries()) {
      ...
      const createTask = (fn: typeof worker.single | typeof worker.movie | typeof worker.batch) =>
        tasks.push(raceWithHandler(
          fn.call(worker, options, extOptions) as Promise<TorrentResult[]>,
      ...
      createTask(worker.single.bind(worker))
      if (checkMovie) createTask(worker.movie.bind(worker))
      if (checkBatch) createTask(worker.batch.bind(worker))
    }
  ...
  async nzbQuery (hash: string, media: Media, episode: number, fileInfo: string | string[], name: string) {
    ...
    const { settled, errors } = await toSettled<ExtensionError, string | undefined>(Array.from(extensions.entries()).map(async ([id, worker]) => {
  ...
  async webSeedQuery (hash: string, media: Media, episode: number, files: WebSeedFile | WebSeedFile[], name: string) {
    ...
    const { settled, errors } = await toSettled<ExtensionError, WebSeedResult | WebSeedResult[] | undefined>(Array.from(extensions.entries()).map(async ([id, worker]) => {
  ...
  async subtitlesQuery (media: Media, episode: number) {
    ...
    const { settled, errors } = await toSettled<ExtensionError, Array<{ url: string, language: string }> | undefined>(Array.from(extensions.entries()).map(async ([id, worker]) => {
