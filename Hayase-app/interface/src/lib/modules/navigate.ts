...
const isTV = typeof window !== 'undefined' && (typeof (window as any).tizen !== 'undefined' || (window as any).native?.isApp)

type InputType = 'mouse' | 'touch' | 'dpad'
export const inputType: Writable<InputType> = writable(isTV ? 'dpad' : 'touch')
...
for (const { pointer, value } of pointerTypes) {
  ...
  if (media.matches && !isTV) inputType.value = value as InputType
  media.addEventListener('change', e => {
    if (e.matches && !isTV) inputType.value = value as InputType
  })
}
...
export function keywrap (cb: (_: KeyboardEvent) => unknown = noop) {
  return (e: KeyboardEvent) => {
    ...
    if (isEnter && inputType.value === 'dpad' && !e.repeat) {
      e.stopPropagation()
      e.preventDefault()
      e.stopImmediatePropagation()
      cb(e)
...
export function click (node: HTMLElement, cb: (_: Event) => unknown = noop) {
  ...
  node.addEventListener('keydown', e => {
    ...
    if (isEnter && inputType.value === 'dpad') {
      e.stopPropagation()
      e.preventDefault()
      cb(e)
    }
...
export function hover (node: HTMLElement, [cb = noop, hoverUpdate = noop]: [typeof noop, (_: boolean) => void]) {
  ...
  node.addEventListener('click', e => {
    e.stopPropagation()
    if (inputType.value === 'dpad' && e.detail === 0) return
    clickElement()
  }, ctrl)
...
const DirectionKeyMap: Record<string, Direction> = { ArrowDown: 'down', ArrowUp: 'up', ArrowLeft: 'left', ArrowRight: 'right', Down: 'down', Up: 'up', Left: 'left', Right: 'right' }
const KeyCodeDirectionMap: Record<number, Direction> = { 38: 'up', 40: 'down', 37: 'left', 39: 'right' }
...
function getKeyboardFocusableElements (element: Element = document.body) {
  return [...element.querySelectorAll<HTMLElement>('a[href]:not([disabled=""]):not([disabled="true"]):not([tabindex="-1"]), button:not([disabled=""]):not([disabled="true"]):not([tabindex="-1"]), fieldset:not([disabled=""]):not([disabled="true"]), input:not([disabled=""]):not([disabled="true"]):not([readonly]), optgroup:not([disabled=""]):not([disabled="true"]), option:not([disabled=""]):not([disabled="true"]), select:not([disabled=""]):not([disabled="true"]), textarea:not([disabled=""]):not([disabled="true"]), details, [tabindex]:not([tabindex="-1"]):not([disabled=""]):not([disabled="true"]), [contenteditable], [controls]')].filter(
    el => !el.getAttribute('aria-hidden') && (el.offsetParent !== null || el.style.position === 'fixed')
  )
}
...
function getElementsInDesiredDirection (keyboardFocusable: ElementPosition[], currentElement: ElementPosition, direction: string): ElementPosition[] {
  ...
  return keyboardFocusable.filter(position => {
    ...
    switch (direction) {
      case 'right': return position.x > currentElement.x + 2 && candidateRect.right > currentRect.right
      case 'left': return position.x < currentElement.x - 2 && candidateRect.left < currentRect.left
      case 'down': return position.y > currentElement.y + 2 && candidateRect.bottom > currentRect.bottom
      case 'up': return position.y < currentElement.y - 2 && candidateRect.top < currentRect.top
  ...
  if (elementsInDesiredDirection.length) {
    ...
    const scored = elementsInDesiredDirection.map(position => {
      ...
      if (isHorizontal) {
        ...
        hasOverlap = overlap >= Math.min(10, minHeight * 0.3)

        const dx = Math.max(0, direction === 'right'
          ? candidateRect.left - currentRect.right
          : currentRect.left - candidateRect.right)
        const dy = Math.abs(position.y - currentElement.y)
        distance = dx + (hasOverlap ? dy * 0.5 : dy * 3)
      } else {
        ...
        const minWidth = Math.min(currentRect.width, candidateRect.width)
        hasOverlap = overlap >= Math.min(20, minWidth * 0.3)

        const dy = Math.max(0, direction === 'down'
          ? candidateRect.top - currentRect.bottom
          : currentRect.top - candidateRect.bottom)
        const dx = Math.abs(position.x - currentElement.x)
        distance = dy + (hasOverlap ? dx * 0.5 : dx * 3)
      }

      return { element: position.element, distance, overlap: hasOverlap, fallbackDistance: getDistance(currentElement, position) }
    })

    const overlapping = scored.filter(s => s.overlap)
    const best = overlapping.length
      ? overlapping.sort((a, b) => a.distance - b.distance)[0]!
      : scored.sort((a, b) => a.distance - b.distance)[0]!
...
function focusElement (element?: HTMLElement | null) {
  ...
  const isTVDevice = isTV || typeof (window as any).tizen !== 'undefined'
  element.scrollIntoView({ block: 'center', inline: 'center', behavior: isTVDevice ? 'auto' : 'smooth' })
