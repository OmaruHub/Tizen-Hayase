...
export async function resolveFilesPoorly (promise: Promise<{media: Media, id: string, episode: number, files: TorrentFile[]}| null>) {
  ...
  if (!resolvedFiles.length) {
    resolvedFiles = await Promise.all(videoFiles.map(file => toResolvedFile(file, list.media)))
  }
...
const AnimeResolver = new class AnimeResolver {
  ...
  async findAnimesByTitle (parseObjects: AnitomyResult[]): Promise<void> {
    ...
    const titleObjects = parseObjects.map(obj => {
      ...
      const altTitles = this.alternativeTitles(obj)
      if (!altTitles.length) return []
      const titleObjects = altTitles.map(title => ({ title, year: obj.anime_year[0], key, isAdult: false }))
      if (titleObjects.length > 0) {
        titleObjects.push({ ...titleObjects[titleObjects.length - 1]!, isAdult: true })
      }
      return titleObjects
    }).flat()
