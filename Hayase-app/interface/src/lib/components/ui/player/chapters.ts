...
async function getChaptersAniSkip (idMal: number, episode: number, duration: number): Promise<Chapter[]> {
  try {
    const resAccurate = await fetch(`https://api.aniskip.com/v2/skip-times/${idMal}/${episode}/?episodeLength=${duration}&types=op&types=ed&types=recap`).catch(() => null)
    const jsonAccurate = resAccurate && resAccurate.ok ? await resAccurate.json().catch(() => null) as AniSkip : null

    const resRough = await fetch(`https://api.aniskip.com/v2/skip-times/${idMal}/${episode}/?episodeLength=0&types=op&types=ed&types=recap`).catch(() => null)
    const jsonRough = resRough && resRough.ok ? await resRough.json().catch(() => null) as AniSkip : null

    const accurateResults = (jsonAccurate && Array.isArray(jsonAccurate.results)) ? jsonAccurate.results : []
    const roughResults = (jsonRough && Array.isArray(jsonRough.results)) ? jsonRough.results : []

    const map: Record<string, Result> = {}
    for (const result of [...accurateResults, ...roughResults]) {
      if (result && !(result.skipType in map)) map[result.skipType] = result
    }
