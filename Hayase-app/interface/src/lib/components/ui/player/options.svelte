...
<Dialog.Root portal={wrapper} bind:open>
  ...
  <Dialog.Content class='absolute bg-transparent border-none p-0 shadow-none size-full overflow-hidden max-w-full options-dialog-content'>
            ...
            <Tree.Item id='audio'>
              ...
              <Tree.Sub>
                {#each Object.entries(normalizeTracks(video.audioTracks ?? [])) as [lang, tracks] (lang)}
                  <Tree.Item id={'audio-' + lang}>
                    <span slot='trigger' class='capitalize'>{lang}</span>
                    <Tree.Sub>
                      {#each tracks as track (track.id)}
                        <Tree.Item id={'audio-track-' + track.id} active={track.enabled} on:click={() => { selectAudio(track.id); close() }}>
            ...
            <Tree.Item id='video'>
              ...
              <Tree.Sub>
                {#each Object.entries(normalizeTracks(video.videoTracks ?? [])) as [lang, tracks] (lang)}
                  <Tree.Item id={'video-' + lang}>
                    <span slot='trigger' class='capitalize'>{lang}</span>
                    <Tree.Sub>
                      {#each tracks as track (track.id)}
                        <Tree.Item id={'video-track-' + track.id} active={track.enabled} on:click={() => { selectVideo(track.id); close() }}>
                          <span>{track.label}</span>
                        </Tree.Item>
                      {/each}
                    </Tree.Sub>
            ...
            <Tree.Item id='subs'>
              ...
              <Tree.Sub>
                ...
                {#each Object.entries(normalizeSubs($tracks)) as [lang, _tracks] (lang)}
                  <Tree.Item id={'subs-' + lang}>
                    <span slot='trigger' class='capitalize'>{lang}</span>
                    <Tree.Sub>
                      {#each _tracks as { number, name }, i (i)}
                        <Tree.Item id={'sub-track-' + number} active={Number(number) === Number($current)} on:click={() => { $current = number; close() }}>
                          <span>{name}</span>
                        </Tree.Item>
                      {/each}
                    </Tree.Sub>
                ...
                {/each}
                <Tree.Item id='subs-add-file' on:click={() => { subtitles.pickFile(); close() }}>
