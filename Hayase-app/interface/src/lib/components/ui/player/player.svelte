<script lang='ts'>
  ...
  let useMediaBunnyPlayback = !SUPPORTS.isTV && !SUPPORTS.isTizen && !SUPPORTS.isTizenTV && (typeof window !== 'undefined' && 'VideoDecoder' in window) && ($settings.playerCustom || dev)
  ...
  $: if (subtitles) {
    subtitles.timeOffset = Number(subtitleDelay)
  ...
  $: isPlayer = $page.route.id === '/app/player' || currentHash.startsWith('#/app/player') || (typeof location !== 'undefined' && location.hash.startsWith('#/app/player'))
  $: isMiniplayer = !isPlayer && !SUPPORTS.isTV && !SUPPORTS.isTizen && !SUPPORTS.isTizenTV
  ...
  async function fullscreen () {
    const target = document.getElementById('episodeListTarget')!
    if (SUPPORTS.isAndroid || SUPPORTS.isTV || SUPPORTS.isTizen || SUPPORTS.isTizenTV) {
      if (fullscreenElement || target.classList.contains('custom-fullscreen')) {
        ...
        return target.requestFullscreen?.({ navigationUI: 'hide' }).catch(() => {})
      }
    }
    try {
      ...
      console.error(err)
      // hacky workaround for android/tv permission system
  ...
  $: fullscreenElement ? screen.orientation.lock?.('landscape')?.catch?.(() => {}) : screen.orientation.unlock?.()
  ...
  onNavigate(({ to, from }) => {
    ...
    if (SUPPORTS.isAndroid || SUPPORTS.isTV || SUPPORTS.isTizen || SUPPORTS.isTizenTV) {
      document.getElementById('episodeListTarget')?.classList.remove('custom-fullscreen')
      document.body.classList.remove('is-fullscreen')
    }
    if (to?.route.id === '/app/player') {
      // force fullscreen on mobile and TV
      if ((SUPPORTS.isMobile && !SUPPORTS.isIPad) || SUPPORTS.isTV || SUPPORTS.isTizen || SUPPORTS.isTizenTV) return fullscreen()
  ...
  function seekBarKey (event: KeyboardEvent) {
    ...
    switch (event.key) {
      ...
      case 'ArrowUp':
        event.preventDefault()
        event.stopPropagation()
        wrapper?.querySelector<HTMLElement>('button, [role="button"]')?.focus()
        break
      case 'Enter':
        playPause()
        break
...
<svelte:window on:hashchange={onHashChange} on:keydown|capture={() => { stopAnimation(); resetMove(5000) }} />
...
>
  ...
  {#if !isMiniplayer}
    <div class='absolute size-full flex items-center justify-center top-0 pointer-events-none'>
      ...
      {/if}
      {#if ($settings.minimalPlayerUI || (SUPPORTS.isMobile && !SUPPORTS.isAndroidTV)) && !SUPPORTS.isTV && !SUPPORTS.isTizen && !SUPPORTS.isTizenTV}
      ...
      {#if !$settings.minimalPlayerUI && (!SUPPORTS.isAndroid || SUPPORTS.isAndroidTV) && !SUPPORTS.isIOS}
        <div class='justify-between gap-2 flex'>
          <div class='flex text-foreground gap-2'>
            <Button id='player-play-btn' class='p-3 size-12 relative shrink-0' variant='ghost' on:click={playPause} on:keydown={keywrap(playPause)}>
              {#if paused}
